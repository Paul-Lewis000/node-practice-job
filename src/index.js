...
module.exports = function({ commitsPerDay, workdaysOnly, startDate, endDate }) {
  const commitDateList = createCommitDateList({
    workdaysOnly: true,
    commitsPerDay: [0, 5],
    startDate: startDate ? parse(startDate) : addYears(new Date(), -5),
  ...
  (async function() {
    ...
    for (const date of commitDateList) {
      ...
      await execAsync(
        `git commit --quiet --date "${date}" -m "tourist Commit"`
      );
    }
...
function createCommitDateList({
  ...
  console.log(commitsPerDay, workdaysOnly, startDate, endDate);
  const commitDateList = [];
  let currentDate = startDate;

  while (currentDate <= endDate) {
    if (
      (workdaysOnly && isWeekend(currentDate) && Math.random() * 10 < 8) ||
      getMonth(currentDate) === 7 ||
